# Piano: Creare seed-db.bat per popolare il database

## Contesto
- Il progetto CineBase ha già un seeder C# in `backend/scripts/FilmApiSeeder/`
- Il seeder usa `DateTime.Today.AddDays(dayOffset)` → le date NON sono statiche, si aggiornano automaticamente
- Serve un `.bat` per eseguire il seeder comodamente
- Il seeder richiede `TMDB_BEARER_TOKEN` in `backend/.env`

## Cosa farà il .bat
1. Verifica che `.env` esista e abbia `TMDB_BEARER_TOKEN`
2. Opzionalmente permette reset (`--reset-shows` o `--reset-all`)
3. Esegue `dotnet run --project backend/scripts/FilmApiSeeder/`
4. Mostra riepilogo finale

## TODO

- [x] 1. Creare `seed-db.bat` nella root del progetto

  **Cosa fare**: Creare `seed-db.bat` con questo contenuto:
  ```batch
  @echo off
  chcp 65001 >nul
  ...
  echo (basate sulla data odierna).
  echo.
  ...
  echo - Programmazione dinamica (7 giorni)
  echo.
  echo Le date degli spettacoli sono calcolate
  echo automaticamente da oggi in avanti.
  echo.
  pause
  ```

  **File**: `CineBase root/seed-db.bat`
  **Categoria**: `quick`

  **Verifica**: Eseguire `seed-db.bat` → parte il seeder

- [x] F1. **Verifica**: Eseguire `seed-db.bat` e controllare che parta senza errori
